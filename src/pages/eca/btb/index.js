...
export default function BTB(props) {
    ...
    const { getBTBHeader, getBTBChildByHeader } = useBtb()
    ...
    const buttonFilterOnClick = (type) => {
        ...
        if (type === 'all') newState({btbHeadTemp: state.btbHead})
        else {
            state.btbHead.map((item) => { 
    ...
    const searchFilterOnChange = (search) => {
        ...
        state.btbHead.map((data) => {
            let dataExists = false

            // Slugify claim header
            let btb = slugify(data.bhid)
            let searchKey = slugify(search)
            
            if (btb.includes(searchKey) && !dataExists) {
                filteredBTB.push(data)
                dataExists = true
            }
        })
        
        newState({
            btbHeadTemp: filteredBTB
        })
    }
    ...
    const dateRangeFilterOnChange = (date) => {
        ...
        state.btbHead.map((data) => {
            ...
            let btbDate = new Date(data.createdt).getTime()
            if (dateRange.firstDate <= btbDate && btbDate <= dateRange.lastDate && !dataExists) {
                dataExists = true
            }

            if (dataExists) filteredBTB.push(data)
        })
    ...
    const fetchData = useCallback( async () => {
        ...
        if (userData.id !== undefined) {
            ...
            setIsLoading(true)
            setTimeout(async () => {
                let listBtbHead = await getBTBHeader(userData.id)
                newState({ 
                    btbHead: listBtbHead,
                    btbHeadTemp: listBtbHead,
                    btbHeaderData: {},
                    fetchStatus: true
                })
                setIsLoading(false)
    ...
    return (
        <LayoutList title="List of BTB" 
            ...
            {  
            state.btbHeadTemp?.length > 0 ? (
                <GridColumn>
                    {
                            state.btbHeadTemp.map((item, index) => {
                                return (
                                    <Dropdown   key={index} 
                                                title={item.bhid}
                                                ...
                                                isLoading={isLoading}
                                                onClick={clicked => {
                                                    if (!state.btbHeaderData.hasOwnProperty(item.bhid)) {
                                                        getBTBHeaderData(item.bhid)
                                                    }
                                                }}
                                                append={ <BadgeStatus type='badge' status={item.status} feature={'btb'} />}>
                                        
                                        { 
                                            !isLoading && (
                                            <>
                                                {
                                                    state.btbHeaderData.hasOwnProperty(item.bhid) ? (
                                                        <>
                                                            <div className="w-full px-4 flex flex-col gap-2">
                                                                            ...
                                                                            <>
                                                                                <div className="w-full grid xl:grid-cols-2 grid-cols-3 gap-1">
                                                                                    ...
                                                                                    <div className="text-white text-xs font-normal">
                                                                                        : IDR {new Intl.NumberFormat('de-DE').format(getAmount(state.btbHeaderData[item.bhid], 'amount'))}
                                                                                    </div>
                                                                                </div>

                                                                                {
                                                                                    getAmount(state.btbHeaderData[item.bhid], 'rejected') !== 0 && (
                                                                                        <>
                                                                                            <div className="w-full grid xl:grid-cols-2 grid-cols-3 gap-1">
                                                                                                <div className="text-white text-xs font-medium">Total Reject</div>
                                                                                                <div className="text-white text-xs font-normal">
                                                                                                    : IDR {new Intl.NumberFormat('de-DE').format(getAmount(state.btbHeaderData[item.bhid], 'rejected'))}
                                                                                                </div>
                                                                                            </div>
                                                                                            {/* <div className="w-full grid xl:grid-cols-2 grid-cols-3 gap-1">
                                                                        ...
                                                                        item.status === 0 && (
                                                                            <>
                                                                                <div className="w-full grid xl:grid-cols-2 grid-cols-3 gap-1">
                                                                                    ...
                                                                                    <div className="text-white text-xs font-normal">
                                                                                        : IDR {new Intl.NumberFormat('de-DE').format(getAmount(state.btbHeaderData[item.bhid], 'paid'))}
                                                                                    </div>
                                                                                </div>
                                                                                {
                                                                                    getAmount(getAmount(state.btbHeaderData[item.bhid], 'rejected')) !== 0 && (
                                                                                        <>
                                                                                            <div className="w-full grid xl:grid-cols-2 grid-cols-3 gap-1">
                                                                                                <div className="text-white text-xs font-medium">Total Reject</div>
                                                                                                <div className="text-white text-xs font-normal">
                                                                ...
                                                                {       
                                                                    state.btbHeaderData[item.bhid].length !== 0 &&  (
                                                                        <>
                                                                            {
                                                                                state.btbHeaderData[item.bhid].map((item, index) => {
                                                                                    return (
                                                                                        <CardBTB key={index} data={item} feature="list"/>
                                                                                    )
                                                                                })
                                                                            }
                                                                        </>
                                                            ...
                                                            <Link href={`/eca/btb/detail?bhid=${item.bhid}`}>
                ...
                <>
                    {
                        state.fetchStatus && (
                            <div className={`w-screen mt-52 flex flex-col place-content-center items-center`}>
                                ...
                                <p className="text-sm mt-2 font-medium">You have no BTB</p>
                            </div>
                        )
                    }
                </>
